 ...
 */
export const getCurrentUser = async (): Promise<{ success: boolean; user: User | null }> => {
  if (USE_DUMMY_DATA) {
    await new Promise(resolve => setTimeout(resolve, 200));
 ...
 * Update user profile
 */
export const updateProfile = async (data: Partial<User>): Promise<{ success: boolean; user: User }> => {
  if (USE_DUMMY_DATA) {
